...
import me.CarsCupcake.SkyblockRemake.Items.trading.ItemTrade;
...
public final class Hex {
    ...
    public void openLandingPage() {
        gui = new GUI(new InventoryBuilder(6, "The Hex")
                .fill(TemplateItems.EmptySlot.getItem())
                .setItem(22, new ItemStack(Material.AIR))
                .setItems(getHexSurrounding(), slots)
                .build());
        gui.setGeneralAction((slot, actionType, type) -> {
            if (actionType == GUI.GUIActions.PlayerClick && !type.isShiftClick())
                return false;
            ...
            for (int l : upgradeSlots) {
                if (l == slot && avaidableOptions.size() > i){
                    avaidableOptions.get(i).factor(Hex.this).showGUI(player);
                    break;
                }
                i++;
            }
