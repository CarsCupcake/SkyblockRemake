...
public class TyphoesT4 extends SkyblockEntity implements Demons, FinalDamageDesider {
    ...
    public void spawn(Location loc) {
        ...
        fireTrail = new BukkitRunnable() {
            final HashMap<SkyblockPlayer, Integer> cooldown = new HashMap<>();

            @Override
            public void run() {
                Block current = entity.getLocation().getBlock();
                if (!blockData.containsKey(current)) {
                ...
                for (SkyblockPlayer player : cooldown.keySet())
                    if (cooldown.get(player) < 1) cooldown.remove(player);
                    else cooldown.put(player, cooldown.get(player) - 1);
    ...
    public void updateTimer() {
        String str;
        if (isInvincible) str = "§4§lIMMUNE ";
        else str = shield.getName() + " " + hits + " ";
    ...
    public void setInvinceble(boolean bol) {
        isInvincible = bol;
        updateTimer();
        if (!isInvincible) startAttack(baseSlayer.owner);
    }
    ...
    public void updateNameTag() {
        if (health > 999) {
            if (health > 9999) {
                ...
                entity.setCustomName("§c" + Character.toChars(9760)[0] + " §b" + name + " §a" + Tools.round((float) ((float) health / 1000), 1) + "k§c§?§");
    ...
    public void kill() {
        super.kill();
        if (baseSlayer != null) baseSlayer.setTyphoeusKilled();
        tpRun.cancel();
        timer.remove();
        aoeRunner.cancel();
        fireTrail.cancel();
    ...
    public void damage(double damage, SkyblockPlayer player) {
        if (!isInvincible) health -= (int) damage;
        if (health > 0) {
