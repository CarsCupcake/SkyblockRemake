...
public class BlazeSlayerT4 extends SkyblockEntity implements FinalDamageDesider, FirePillar.PillarThrower, FirePits.FirePitExecuter {

	
	private int health = 150000000;
		...
		if(!demonsplitHasActivated && health >0) {
			if((health <= getMaxHealth() * 0.66 && demonsplitPhase == 0) || (health <= getMaxHealth() * 0.33 && demonsplitPhase == 1)) {
				...
				if(demonsplitPhase == 1)
					new FirePits(BlazeSlayerT4.this, owner, false);
