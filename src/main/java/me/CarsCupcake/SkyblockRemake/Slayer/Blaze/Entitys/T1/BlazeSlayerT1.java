            ...
            if (health > 9999) {
                if (health > 999999) {
                    if (health > 9999999) {
                        ...
                        entity.setCustomName("§c" + Character.toChars(9760)[0] + " §b" + name + " §a" + Tools.round((float) health / 1000000, 1) + "m§c§?§");
            ...
            public void run() {
                ...
                if (ticks == 20) {
                    ...
                    if (time == 0) {
                        ...
                        if (stand != null) stand.setCustomName("§c" + shortInteger(time));
...
//					typhoes.startAttack(SkyblockPlayer.getSkyblockPlayer(owner));
    ...
    public int getTrueDamage() {
        ...
        double mult;
        if (getMaxHealth() * 0.33 > health) mult = 0.12;
        else if (getMaxHealth() * 0.66 > health) mult = 0.08;
        else mult = 0.05;
    ...
    private void startAoe() {
        aoeRunner = new BukkitRunnable() {
            ...
            public void run() {
                ...
                float trueehp = (float) Main.getPlayerStat(owner, Stats.Health) * (1 + ((float) Main.getPlayerStat(owner, Stats.Defense) / 100));
                float effectivetruedmg = (float) Main.getPlayerStat(owner, Stats.Health) / trueehp;
                int totaldmg = (int) (truedamage * effectivetruedmg);
