...
public class BlazeSlayerT3 extends SkyblockEntity implements FinalDamageDesider, FirePillar.PillarThrower, FirePits.FirePitExecuter {

	
	private int health = 45000000;
	...
	@Getter
	private QuaziiT3 quazii;
	@Getter
	private TyphoesT3 typhoes;
	...
	public void demonsplit(int beforhealth) {
		if(!demonsplitHasActivated && health >0) {
			if((health <= getMaxHealth() * 0.66 && demonsplitPhase == 0) || (health <= getMaxHealth() * 0.33 && demonsplitPhase == 1)) {
				...
				isInvincible = true;
				if(demonsplitPhase == 1)
					new FirePits(BlazeSlayerT3.this, owner, false);
				...
				new BukkitRunnable() {
					...
					QuaziiT3 quazii = new QuaziiT3();
					quazii.setBlazeSlayer(BlazeSlayerT3.this);
					quazii.spawn(quazziLoc);
//					quazii.startAttack(SkyblockPlayer.getSkyblockPlayer(owner));
					quaziiAlive = true;
					BlazeSlayerT3.this.quazii = quazii;
					quaziiStand.setPassenger(quazii.getEntity());
					
					TyphoesT3 typhoes = new TyphoesT3();
					typhoes.setBlazeSlayer(BlazeSlayerT3.this);
...
//					typhoes.startAttack(SkyblockPlayer.getSkyblockPlayer(owner));
					...
					BlazeSlayerT3.this.typhoes = typhoes;
	...
	private void summounBlazeSlayerBack(SkyblockEntity e) {
		...
		if(demonsplitPhase == 2){
			new FirePits(BlazeSlayerT3.this, owner, true);
		}
