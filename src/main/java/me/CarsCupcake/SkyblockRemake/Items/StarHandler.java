...
public class StarHandler {
    public static String getStarSuffix(ItemStack item){
        ...
        if(!ItemHandler.hasPDC("stars",item, PersistentDataType.INTEGER))
            return "";
        int stars = ItemHandler.getPDC("stars", item, PersistentDataType.INTEGER);
        if(stars <= 0)
            return "";
        String s = "";
        if(stars <= 5){
    ...
    public static void setStars(ItemStack item, int i){
        Assert.allNotNull("", item, item.getItemMeta(), item.getItemMeta().getPersistentDataContainer());
