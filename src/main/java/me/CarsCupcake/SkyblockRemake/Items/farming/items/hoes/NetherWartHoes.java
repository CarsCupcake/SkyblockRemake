...
public class NetherWartHoes implements Listener {
    public NetherWartHoes(){
        AbilityLore lore =  new AbilityLore(new ArrayList<>(List.of("§7Counter: §e%counter% nether warts", " ", "%next%")));
        ...
        AbilityLore boosts = new AbilityLore(new ArrayList<>(List.of("§7Harvest §a+10% nether warts §7and",
                "§7gain " + Stats.FarmingWisdom.getColor() + "+%wisdom% " + Stats.FarmingWisdom.getSymbol() + " Farming Wisdom",
                "§7while harvesting nether warts.")));
        boosts.addPlaceholder("%wisdom%", (player, itemStack) -> getFarmingWisdomStatBoos(itemStack, player) + "");
        ItemManager manager = new ItemManager("Newton Nether Warts Hoe", "THEORETICAL_HOE_WARTS_1", ItemType.Hoe, Material.STONE_HOE, ItemRarity.COMMON);
        manager.setRarityGrabber(new UpgradebleHoe(Material.NETHER_WART));
        ...
        AbilityLore logarithmicCounter = new AbilityLore(new ArrayList<>(List.of(
                "§7Harvest §a+16% §7nether warts per",
        ...
        boosts = new AbilityLore(new ArrayList<>(List.of("§7Harvest §a+25% nether warts §7and",
                "§7gain " + Stats.FarmingWisdom.getColor() + "+%wisdom% " + Stats.FarmingWisdom.getSymbol() + " Farming Wisdom",
                "§7while harvesting nether warts.")));
        boosts.addPlaceholder("%wisdom%", (player, itemStack) -> getFarmingWisdomStatBoos(itemStack, player) + "");

        manager = new ItemManager("Newton Nether Warts Hoe", "THEORETICAL_HOE_WARTS_2", ItemType.Hoe, Material.IRON_HOE, ItemRarity.UNCOMMON);
        manager.setRarityGrabber(new UpgradebleHoe(Material.NETHER_WART));
        manager.addAbility(null, null, null, boosts, 0, 0);
        manager.addAbility(null, null, null, lore, 0, 0);
        ...
        AbilityLore collectionAnalasys = new AbilityLore(new ArrayList<>(List.of(
                "§7Harvest §a+8% §7nether warts per digits",
                "§7of your collection, minus 4!",
                "§a+%a% §6" + Stats.FarmingFortune.getSymbol() + " Farming Fortune §7for",
                "§7nether warts."
        )));
        collectionAnalasys.addPlaceholder("%a%", (player, itemStack) -> ((player != null) ? FarmingUtils.getCollectionAnalysis(ItemCollection.itemCollections.get(player).get(Material.NETHER_WART.toString())) : "0") + "");

        boosts = new AbilityLore(new ArrayList<>(List.of("§7Harvest §a+50% nether warts §7and",
                "§7gain " + Stats.FarmingWisdom.getColor() + "+%wisdom% " + Stats.FarmingWisdom.getSymbol() + " Farming Wisdom",
                "§7while harvesting nether warts.")));
        boosts.addPlaceholder("%wisdom%", (player, itemStack) -> getFarmingWisdomStatBoos(itemStack, player) + "");

        manager = new ItemManager("Newton Nether Warts Hoe", "THEORETICAL_HOE_WARTS_3", ItemType.Hoe, Material.DIAMOND_HOE, ItemRarity.RARE);
        manager.setRarityGrabber(new UpgradebleHoe(Material.NETHER_WART));
        manager.addAbility(null, null, null, boosts, 0, 0);
        manager.addAbility(null, null, null, lore, 0, 0);
        manager.addAbility(null, null, "Logarithmic Counter", logarithmicCounter, 0, 0);
    ...
    public void onFarmEvent(PlayerFarmEvent event){
        ...
        switch (manager.itemID.charAt(manager.itemID.length() - 1)){
            ...
            case '3' -> event.setFarmingFortune(event.getFarmingFortune() + 50 + FarmingUtils.getLogarithmicCounter(counter) + FarmingUtils.getCollectionAnalysis(ItemCollection.itemCollections.get(event.getPlayer()).get(Material.NETHER_WART + "")));
        }
