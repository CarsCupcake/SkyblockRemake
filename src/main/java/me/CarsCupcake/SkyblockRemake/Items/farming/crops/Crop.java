...
public abstract class Crop {
    ...
    public ItemStack makeDrop(SkyblockPlayer player, Block b) {
        ...
        PlayerFarmEvent event = new PlayerFarmEvent(player, b, Main.getPlayerStat(player, Stats.FarmingFortune), Main.getPlayerStat(player, Stats.FarmingWisdom), (Objects.equals(d.getFirst(), d.getLast())) ? d.getFirst() : new Random().nextInt(d.getLast() - d.getFirst()) + d.getFirst());
        ...
        if (rest != 0) {
            if (new Random().nextDouble() <= rest) mult += 1;
        }
