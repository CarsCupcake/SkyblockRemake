...
public class PotatoHoes implements Listener {
    public PotatoHoes(){
        AbilityLore lore =  new AbilityLore(new ArrayList<>(List.of("§7Counter: §e%counter% Potatoes", " ", "%next%")));
        ...
        AbilityLore boosts = new AbilityLore(new ArrayList<>(List.of("§7Harvest §a+10% potatoes §7and",
        ...
        boosts = new AbilityLore(new ArrayList<>(List.of("§7Harvest §a+25% potatoes §7and",
                "§7gain " + Stats.FarmingWisdom.getColor() + "+%wisdom% " + Stats.FarmingWisdom.getSymbol() + " Farming Wisdom",
                "§7while harvesting potatoes.")));
        boosts.addPlaceholder("%wisdom%", (player, itemStack) -> getFarmingWisdomStatBoos(itemStack, player) + "");
    ...
    public void onFarmEvent(PlayerFarmEvent event){
        ...
        switch (manager.itemID.charAt(manager.itemID.length() - 1)){
            ...
            case '3' -> event.setFarmingFortune(event.getFarmingFortune() + 50 + FarmingUtils.getLogarithmicCounter(counter) + FarmingUtils.getCollectionAnalysis(ItemCollection.itemCollections.get(event.getPlayer()).get(Material.POTATO + "")));
        }
