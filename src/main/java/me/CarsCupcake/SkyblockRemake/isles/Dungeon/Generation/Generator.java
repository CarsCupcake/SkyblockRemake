...
public final class Generator extends MapRenderer {
    ...
    public Generator(){
        ...
        for (int i = 1; i < 4; i++)
            map.put(rL.get(i - 1), new Room(DungeonRoomsTypes.puzzle, rL.get(i), null, 0));
        ...
        for (Location2d loc : map.getEmpty()){
            ...
            if(isFitting(type, loc, rotation)){
                Room r = new Room(type, loc, managers.get(type).getNewRandom(new HashSet<>()), 0);
                map.put(loc, r);
        ...
        for (Location2d loc : map.getEmpty()){
            ...
            if(isFitting(type, loc, rotation)){
                {
                    map.put(new Room(type, loc, managers.get(type).getNewRandom(new HashSet<>()), rotation));
                }
            }else
    ...
    public void place(){
        new BukkitRunnable() {
            public void run(){
                ...
                Bukkit.broadcastMessage("§aThis operation took " + (System.currentTimeMillis() - before) + "ms");
            }
