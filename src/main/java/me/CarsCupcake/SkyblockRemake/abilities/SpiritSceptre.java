...
public class SpiritSceptre implements AbilityManager<PlayerInteractEvent> {
    @Override
    ...
    private void explode(SpiritBat bat, SkyblockPlayer player){
        bat.kill();
        int i = 0;
        double damage = 0;
        ...
        for(Entity entity : entities){
            if(entity instanceof LivingEntity && !(entity instanceof  Player) && !entity.equals(bat.getEntity())){
                ...
                if(SkyblockEntity.livingEntity.exists(e) && SkyblockEntity.livingEntity.getSbEntity(e) instanceof SpiritBat)
                    return;
